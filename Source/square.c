...
float absolute(float x){ //Returns the absolute value of a float number
    if(x < 0) return -x;
...
int main(int argc , char** argv){
    ...
    printf("\nSquare with center(%f,%f),radius=%f,offset=%d,PointsToRead=%d\n\n",center_x,center_y,radius,offset,PointsToReadCount);
    ...
    for (i=0; i<numOfrecords ; i++) {
       	...
       	//printf("%f %f\n", rec.x, rec.y);
        if( absolute(rec.x-center_x)+absolute(rec.y-center_y) <= radius ){  //if point is inside the square then print it to output file
            fprintf(fo, "%f\t%f\n",rec.x,rec.y);
        }
   	 }
    fclose(fp);
