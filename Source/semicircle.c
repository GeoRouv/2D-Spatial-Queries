...
int main(int argc , char** argv){
    ...
    char* inputfile=NULL,*outputfile=NULL,*orientation;
	...
	for(z=0; z<argc; z++){						//Getting tokens from command line
		if(!(strcmp(argv[z],"-a"))){
            ...
            orientation = malloc(strlen(argv[z+4])+1);
            strcpy(orientation,argv[z+4]);
    ...
    printf("\nSemicircle with center(%f,%f),radius=%f and orientation=%s,offset=%d,PointsToRead=%d\n\n",center_x,center_y,radius,orientation,offset,PointsToReadCount);
    ...
    for (i=0; i<numOfrecords ; i++) {
       	...
       	//printf("%f %f\n", rec.x, rec.y);
        if((rec.x - center_x)*(rec.x - center_x) + (rec.y - center_y)*(rec.y - center_y) <= radius*radius){  //if point is inside the semicircle then print it to output file
            if(!(strcmp(orientation,"N")) && rec.y >= center_y) fprintf(fo, "%f\t%f\n",rec.x,rec.y);
            else if(!(strcmp(orientation,"S")) && rec.y <= center_y) fprintf(fo, "%f\t%f\n",rec.x,rec.y);
            else if(!(strcmp(orientation,"E")) && rec.x >= center_x) fprintf(fo, "%f\t%f\n",rec.x,rec.y);
            else if(!(strcmp(orientation,"W")) && rec.x <= center_x) fprintf(fo, "%f\t%f\n",rec.x,rec.y);
        }
   	 }
